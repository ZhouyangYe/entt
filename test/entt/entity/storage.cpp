...
TEST(Storage, StableErase) {
    ...
    pool.stable_erase(std::begin(entities), std::end(entities));

    ASSERT_DEATH(pool.stable_erase(std::begin(entities), std::end(entities)), "");
    ASSERT_FALSE(pool.empty());
    ...
    ASSERT_EQ(pool.get(entities[0u]), 0);
    ASSERT_EQ(pool.get(entities[1u]), 1);
    ASSERT_EQ(pool.get(entities[2u]), 2);
    ...
    pool.stable_erase(entities, entities + 2u);
    ...
    pool.stable_erase(entities[2u]);

    ASSERT_DEATH(pool.stable_erase(entities[2u]), "");
    ASSERT_DEATH(pool.erase(entities[2u]), "");
    ASSERT_FALSE(pool.empty());
    ASSERT_EQ(pool.size(), 3u);
    ...
    ASSERT_EQ(pool.get(entities[2u]), 1);
    ...
    pool.emplace(entities[0u], 0);
    pool.emplace(entities[1u], 2);
    pool.emplace(entities[2u], 1);
    pool.stable_erase(entities[2u]);

    ASSERT_DEATH(pool.stable_erase(entities[2u]), "");

    // erase should fall back to stable_erase if a pool contains holes
    pool.erase(entities[0u]);
    pool.stable_erase(entities[1u]);

    ASSERT_DEATH(pool.stable_erase(entities, entities + 2u), "");
    ASSERT_EQ(pool.size(), 3u);
    ASSERT_TRUE(pool.at(2u) == entt::tombstone);

    pool.emplace(entities[0u], 99);

    ASSERT_EQ(*++pool.begin(), 99);

    pool.emplace(entities[1u], 2);
    pool.emplace(entities[2u], 1);
    pool.emplace(entt::entity{0}, 7);

    ASSERT_EQ(pool.size(), 4u);
    ASSERT_EQ(*pool.begin(), 7);
    ASSERT_EQ(pool.at(0u), entities[1u]);
    ASSERT_EQ(pool.at(1u), entities[0u]);
    ASSERT_EQ(pool.at(2u), entities[2u]);

    ASSERT_EQ(pool.get(entities[0u]), 99);
    ASSERT_EQ(pool.get(entities[1u]), 2);
    ASSERT_EQ(pool.get(entities[2u]), 1);
}
...
TEST(Storage, StableRemove) {
    ...
    ASSERT_EQ(pool.stable_remove(std::begin(entities), std::end(entities)), 3u);
    ASSERT_EQ(pool.stable_remove(std::begin(entities), std::end(entities)), 0u);
    ASSERT_FALSE(pool.empty());
    ASSERT_EQ(pool.size(), 3u);
    ASSERT_TRUE(pool.at(2u) == entt::tombstone);
    ASSERT_DEATH([[maybe_unused]] auto &&value = pool.get(entt::tombstone), "");
    ...
    ASSERT_EQ(pool.stable_remove(entities, entities + 2u), 2u);
    ASSERT_FALSE(pool.empty());
    ASSERT_EQ(pool.size(), 3u);
    ASSERT_EQ(*pool.begin(), 2);
    ASSERT_EQ(pool.index(entities[2u]), 2u);

    ASSERT_EQ(pool.stable_remove(entities[2u]), 1u);
    ASSERT_EQ(pool.stable_remove(entities[2u]), 0u);
    ASSERT_EQ(pool.remove(entities[2u]), 0u);
    ASSERT_FALSE(pool.empty());
    ASSERT_EQ(pool.size(), 3u);
    ASSERT_FALSE(pool.contains(entities[0u]));
    ...
    // remove should fall back to stable_remove if a pool contains holes
    ASSERT_EQ(pool.remove(entities[0u]), 1u);
    ASSERT_EQ(pool.stable_remove(entities[1u]), 1u);
    ASSERT_EQ(pool.stable_remove(entities, entities + 2u), 0u);
...
TEST(Storage, Compact) {
    ...
    pool.stable_erase(entt::entity{0});
    ...
    ASSERT_EQ(pool.get(entt::entity{42}), 42);
    ASSERT_EQ(pool.get(entt::entity{0}), 0);

    pool.stable_erase(entt::entity{0});
    pool.stable_erase(entt::entity{42});
    pool.compact();
