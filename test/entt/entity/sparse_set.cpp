...
TEST(SparseSet, Contains) {
    ...
    set.stable_erase(entt::entity{0});
    set.erase(entt::entity{3});

    set.stable_remove(entt::entity{42});
...
TEST(SparseSet, StableErase) {
    ...
    ASSERT_DEATH(set.stable_erase(std::begin(entities), std::end(entities)), "");
    ASSERT_DEATH(set.stable_erase(entities[1u]), "");

    ASSERT_TRUE(set.empty());
    ASSERT_EQ(set.size(), 0u);

    set.insert(std::begin(entities), std::end(entities));
    set.stable_erase(set.begin(), set.end());
    ...
    set.insert(std::begin(entities), std::end(entities));
    set.stable_erase(entities, entities + 2u);
    ...
    set.stable_erase(entities[2u]);

    ASSERT_DEATH(set.stable_erase(entities[2u]), "");
    ASSERT_DEATH(set.erase(entities[2u]), "");
    ASSERT_FALSE(set.empty());
    ASSERT_EQ(set.size(), 6u);
    ...
    set.stable_erase(entities[1u]);

    ASSERT_DEATH(set.stable_erase(entities, entities + 2u), "");
    ASSERT_EQ(set.size(), 3u);
...
TEST(SparseSet, StableRemove) {
    ...
    ASSERT_EQ(set.stable_remove(std::begin(entities), std::end(entities)), 0u);
    ASSERT_EQ(set.stable_remove(entities[1u]), 0u);
    ...
    ASSERT_EQ(set.stable_remove(set.begin(), set.end()), 3u);
    ASSERT_FALSE(set.empty());
    ASSERT_EQ(set.size(), 3u);
    ASSERT_TRUE(set.at(0u) == entt::tombstone);
    ASSERT_TRUE(set.at(1u) == entt::tombstone);
    ...
    ASSERT_EQ(set.stable_remove(entities, entities + 2u), 2u);
    ASSERT_FALSE(set.empty());
    ASSERT_EQ(set.size(), 6u);
    ...
    ASSERT_EQ(set.stable_remove(entities[2u]), 1u);
    ASSERT_EQ(set.stable_remove(entities[2u]), 0u);
    ASSERT_EQ(set.remove(entities[2u]), 0u);
    ASSERT_EQ(set.remove(entities[2u]), 0u);
    ASSERT_FALSE(set.empty());
    ASSERT_EQ(set.size(), 6u);
    ...
    ASSERT_EQ(set.stable_remove(std::begin(entities), std::end(entities)), 2u);
    ASSERT_FALSE(set.empty());
    ...
    ASSERT_EQ(set.stable_remove(entities[1u]), 1u);
    ASSERT_EQ(set.stable_remove(entities, entities + 2u), 0u);
...
TEST(SparseSet, Compact) {
    ...
    set.stable_erase(entt::entity{0});
    set.stable_erase(entt::entity{42});
    set.compact();
